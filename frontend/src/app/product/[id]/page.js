    ...
    <div className="min-h-screen bg-[#f1f3f6] py-4 px-2 sm:px-4" style={{fontFamily:"'Segoe UI',Roboto,Arial,sans-serif"}}>
      <div className="max-w-5xl mx-auto">
        ...
        <nav className="mb-3 text-xs text-gray-500 px-1">
          <span className="hover:text-blue-600 cursor-pointer" onClick={() => window.history.back()}>← Back</span>
          <span className="mx-1">/</span>
          <span className="text-gray-700">{product.category}</span>
          <span className="mx-1">/</span>
          <span className="text-gray-900 font-medium">{product.name?.slice(0,40)}{product.name?.length > 40 ? '…' : ''}</span>
        </nav>
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-sm">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-[44%] border-r border-gray-100 flex flex-col">
              ...
              {/* Inner: thumbnail strip + main image side by side */}
              <div className="flex flex-row flex-1">
                ...
                {images.length > 1 && (
                  <div className="flex flex-col gap-2 p-3 border-r border-gray-100 overflow-y-auto max-h-[480px]">
                    {images.map((img, idx) => (
                      ...
                      >
                        <img src={img} alt={`thumb-${idx}`} className="w-full h-full object-contain p-1" onError={e => { e.target.src = "/no-image.png"; }} />
                      </button>
                    ))}
                  </div>
                )}
                ...
                <div className="flex-1 flex flex-col items-center justify-center relative p-4 min-h-[420px]">
                  ...
                  {/* Zoom hint */}
                  <span className="absolute bottom-3 right-3 text-xs text-gray-400 select-none hidden lg:block">
                    🔍 Hover to zoom
                  </span>
            ...
            <div className="lg:w-[56%] p-5 lg:p-7 flex flex-col gap-3">
              ...
              {/* Title */}
              <h1 className="text-xl sm:text-2xl font-normal text-gray-800 leading-snug">
                {product.name}
              </h1>
